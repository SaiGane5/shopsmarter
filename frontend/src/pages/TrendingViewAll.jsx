...
const TrendingViewAll = () => {
  ...
  // Enhanced offer calculation for trending products
  const calculateOffer = useCallback((product, sectionType) => {
    ...
    // Trending items get higher discounts (20-50%)
    const trendingDiscounts = [20, 25, 30, 35, 40, 45, 50];
    const discount = trendingDiscounts[productSeed % trendingDiscounts.length];
    const badge = `Save ${discount}%`;
  ...
  const applyFilters = useCallback(() => {
    ...
    // Add trending offer details to products
    filtered = filtered.map(product => ({
      ...product,
      sectionType: 'trending',
      offerDetails: calculateOffer(product, 'trending')
    }));
    ...
    switch (filters.sortBy) {
      ...
      case 'rating':
        ...
        break;
      case 'popularity':
        filtered.sort((a, b) => (b.offerDetails?.discount || 0) - (a.offerDetails?.discount || 0));
        break;
      default:
        break;
    }
  ...
  const fetchAdditionalProducts = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/products/trending?limit=50');
      if (response.ok) {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'} transition-colors duration-300`}>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  ...
                  <span className="ml-2 text-xs">• Popular items with huge discounts</span>
                </p>
              </div>
            </div>
            ...
            <div className="p-8">
              <div className="flex items-center justify-between mb-8">
                <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} flex items-center`}>
                  <span className="mr-2">🔥</span> Trending Filters
                </h2>
                <button
                  onClick={resetFilters}
                  className="text-orange-600 hover:text-orange-700 text-sm font-semibold transition-colors hover:scale-105 transform duration-200"
                >
                  Reset All
                </button>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    } focus:ring-2 focus:ring-orange-500 focus:border-transparent shadow-sm hover:shadow-md`}
                  >
                ...
                {availableCategories.length > 0 && (
                  <div>
                    ...
                    <div 
                      className={`space-y-3 max-h-48 overflow-y-auto scrollbar-hide ${isScrolling ? 'scrolling' : ''}`}
                      onScroll={handleScroll}
                    >
                ...
                <div>
                  <label className="flex items-center py-3 cursor-pointer hover:bg-opacity-50 rounded-lg px-2 transition-colors">
                    <input
                      ...
                      className="text-orange-600 focus:ring-orange-500 focus:ring-2 rounded"
                    />
            ...
            {loading ? (
              <div className="flex items-center justify-center py-20">
                <div className="flex flex-col items-center space-y-4">
                  ...
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading trending products...</p>
                </div>
              </div>
              ...
              <div className="text-center py-20">
                ...
                <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-6 max-w-md mx-auto`}>
                  We couldn't find any trending products matching your criteria. Try adjusting your filters.
                </p>
                <button
                  onClick={resetFilters}
                  className="bg-orange-600 text-white px-8 py-3 rounded-xl hover:bg-orange-700 transition-colors font-medium"
                >
                  Reset All Filters
                </button>
              </div>
      ...
      {/* ENHANCED: Auto-Hide Scrollbar Styles for Trending (Orange Theme) */}
      <style jsx>{`
        .scrollbar-hide {
...
// Product Card Component (same as before but with orange hover effects)
const ProductCard = React.memo(({ product, index, cart, addToCart, updateCartQuantity, removeFromCart, navigate, darkMode, sectionType, calculateOffer }) => {
  ...
  const offerDetails = useMemo(() => {
    ...
    return calculateOffer ? calculateOffer(product, sectionType) : {
      discount: 25,
      badge: 'Save 25%',
      originalPrice: parseFloat(product.price || 0) * 1.33,
      currentPrice: parseFloat(product.price || 0),
      savings: parseFloat(product.price || 0) * 0.33
    };
  ...
  return (
    <motion.div
      ...
      className={`group relative cursor-pointer overflow-hidden rounded-3xl transition-all duration-300 ${
        darkMode
          ? "bg-gradient-to-br from-gray-800/80 to-gray-900/60 border border-gray-700/50 hover:border-orange-600 hover:shadow-2xl hover:shadow-orange-900/20"
          : "bg-gradient-to-br from-white to-gray-50/50 border border-gray-200/50 shadow-lg hover:border-orange-300 hover:shadow-2xl hover:shadow-orange-900/10"
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-gray-100 rounded-t-3xl">
        ...
        <div className="absolute top-4 right-4">
          <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white text-xs font-bold px-3 py-2 rounded-xl shadow-lg backdrop-blur-sm">
            {offerDetails.badge}
          </div>
        </div>

        {/* Hot Deal Badge for high discounts */}
        {offerDetails.discount >= 40 && (
          <div className="absolute top-4 left-4">
            <div className="bg-gradient-to-r from-yellow-400 to-orange-500 text-black text-xs font-bold px-3 py-2 rounded-xl shadow-lg">
              🔥 Hot Deal
            </div>
          </div>
        )}
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="h-12 flex items-start">
          <h3
            className={`text-base font-bold leading-tight line-clamp-2 group-hover:text-orange-600 transition-colors duration-200 ${
